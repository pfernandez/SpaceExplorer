...
c  potential solver common blocks
     ...
     1  g(jmax2),h(kmax2)
      ...
      write(6,12)
 12   format(' version 10/9/2010 gfortran compiler only')
      write(6,10)
 10   format(' data file')
      read(5,11) filen
      write(6,17) filen
...
c      do k = 2,kmax
c         do j = 2,jmax
c            write(47,498) j,k,rho(j,k)
c         end do
c         write(47,*)
c      end do
c 498  format(2i5,1p1e12.5)
c
c      write(2,*) jout,kout
c      write(2,*) r(jout),starm
c      write(2,96) ((rho(j,k),j=2,jmax1),k=2,kmax1)
c      write(2,96) (angold(j),j=2,jmax1)
c 96   format(1p6e13.5)
...
c.....calculate corotation radius
     ...
     1                   jin(2),jmax,kmax,lmax,tstepPower,
     1                   toverw,
...
c                  time averaged = .5(drho dphi*)re
     ...
     1         ee(2,j,k)*ee(6,j,k))
          ...
          do j = jin(2)+2,jout(2)
            if(torque(j+1).gt.0.0) then
              if(torque(j).lt.0.0) then
                rTorque1 = rhf(j)/rmax
                write(50,5117) j,rhf(j)/rmax,amassj(j),angmom(j)/totJ
                  exit
              end if 
            end if
          end do
          do j = jin(2)+2,jout(2)
            if(torque(j+1).lt.0.0) then
              if(torque(j).gt.0.0) then
                rTorque2 = rhf(j)/rmax
                write(50,5118) j,rhf(j)/rmax,amassj(j),angmom(j)/totJ
                  exit
              end if
            end if
          flush(50)
          ...
          taumax = scale
          wabs = abs(wsum)
          enorm = 8*pi*y2Avg*anorm/(wabs/vol)
...
c          end do
          ...
          do j = 2,jmax2
            if(abs(wintg(1,j,2)).le.1.0E-30) wintg(1,j,2) = 0.0
            if(abs(wintg(1,j,2)).le.1.0E-30) wintg(1,j,2) = 0.0
            if(abs(wintstr(1,j,2)).le.1.0E-30) wintstr(1,j,2) = 0.0
            if(abs(wintstr(2,j,2)).le.1.0E-30) wintstr(2,j,2) = 0.0
            if(abs(wintstr(3,j,2)).le.1.0E-30) wintstr(3,j,2) = 0.0
            if(abs(djay(j,2)).le.1.0E-30) djay(j,2) = 0.0
            if(abs(torque(j)).le.1.0E-30) torque(j) = 0.0
          end do
          do j = 2,jmax2
...
c            torque(j) = torque(j)*anorm
     ...
     1                     + wintstrN(3,j,2)
            ...
            if(abs(wintgN(1,j,2)).le.1.0E-30) wintgN(1,j,2) = 0.0
            if(abs(wintgN(2,j,2)).le.1.0E-30) wintgN(2,j,2) = 0.0
            if(abs(wintstrN(1,j,2)).le.1.0E-30) wintstrN(1,j,2) = 0.0
            if(abs(wintstrN(2,j,2)).le.1.0E-30) wintstrN(2,j,2) = 0.0
            if(abs(wintstrN(3,j,2)).le.1.0E-30) wintstrN(3,j,2) = 0.0
     ...
     1                     wintstr(3,j,2)
            ...
            write(66,5107) rnorm(j),torque(j),amassj(j),angmom(j)/totJ
            flush(27)
            flush(28)
            flush(29)
            flush(66)
 ...
 5117  format(1x,'    j torque1:  ',1i5,/,
     1        1x,'    R torque1:  ',1pe12.4,/,
     1        1x,'    M torque/M1:  ',1pe12.4,/,
     2        1x,'    Jtorque/J1: ',1pe12.4)
 5118  format(1x,'    j torque2:  ',1i5,/,
     1        1x,'    R torque2:  ',1pe12.4,/,
     1        1x,'    M torque/M2:  ',1pe12.4,/,
     2        1x,'    Jtorque/J2: ',1pe12.4)
